...
#: Scope label for the native "CAS snapshot with no portfolio key" source. Such
#: rows share one native source identity and cannot be attributed to any
#: upload's portfolio key, so they are reported under this explicit label rather
#: than borrowing one.
NULL_PORTFOLIO_LABEL = "(no portfolio key)"
...
class _NetWorthScopeSources(NamedTuple):
    ...
    #: Every CAS upload id mapped to its normalized display key. Coverage is
    #: decided per upload ID (two uploads can share a portfolio key), while the
    #: key is used only for display.
    cas_upload_display_keys: dict[int, str]
    manual_asset_labels: tuple[str, ...]
    manual_liability_labels: tuple[str, ...]
...
class _PortfolioValuation(NamedTuple):
    ...
    ``portfolio_key`` is the exact ``BalanceSnapshot.portfolio_key``, which may
    be ``None`` for legacy/imported rows. ``None`` is a real source identity that
    native net worth groups as one, so it is preserved rather than substituted.
    """
...
async def _load_portfolio_valuations(
...
) -> tuple[dict[str | None, _PortfolioValuation], frozenset[int]]:
    ...
    # ---- winner selection, mirroring native exactly -----------------------
    #
    # Grouped by the RAW portfolio key, as :func:`networth._source_key` does —
    # never a normalized form. Native treats two raw keys differing only in case
    # or whitespace as separate sources and SUMS them, so normalizing here would
    # merge their series and (for same-date rows) drop one.
    #
    # Currency is deliberately NOT filtered in SQL. Native selects the latest row
    # per source FIRST and only then ignores it if it is not INR, so a newer
    # non-INR row makes the source contribute nothing. Filtering in SQL would
    # leave an older INR value standing that native has already dropped.
    #
    # Coverage is derived from the WINNER only: a row that lost same-date
    # selection is not authoritative, so its upload must not count as covered.
    winners: dict[tuple[str | None, datetime.date], BalanceSnapshot] = {}
    for row in rows:
        if as_of is not None and row.as_of_date > as_of:
            continue
        # The EXACT snapshot key — including None, "" and whitespace-only. Each
        # is a real native source identity whose value native includes.
        key = (row.portfolio_key, row.as_of_date)
        current = winners.get(key)
        # Rows arrive ascending by (as_of_date, id), so a later id on the same
        # date supersedes an earlier one — the same rule as _latest_per_source.
        if current is None or (row.id or 0) >= (current.id or 0):
            winners[key] = row

    per_portfolio: dict[str | None, dict[datetime.date, Decimal]] = {}
    covered_upload_ids: set[int] = set()
    for (portfolio, as_of_date), row in winners.items():
        if row.cas_upload_id is not None:
            covered_upload_ids.add(row.cas_upload_id)
        # A non-INR winner contributes ZERO at its date, which clears any older
        # INR balance — exactly what native's post-selection currency check does.
        value = Decimal(row.value) if row.currency == INR else Decimal("0.00")
        per_portfolio.setdefault(portfolio, {})[as_of_date] = value
...
async def _load_net_worth_scope_sources(
    ...
    return _NetWorthScopeSources(
        ...
        cas_upload_display_keys={
            upload.id: normalize_portfolio_key(upload.portfolio_key)
            for upload in uploads
...
async def project(
    ...
    if cas_portfolios_projected:
        ...
        #: The reporting identity of every snapshot-backed series, whether or
        #: not it produced an entry (an all-zero history is still represented).
        snapshot_backed_identities: set[str] = set()
        null_source_projected = False
        # ``None`` sorts before any str, so key on (is_not_null, key) to keep a
        # total order without comparing None to str.
        for raw_portfolio, series in sorted(
            portfolio_values.items(), key=lambda kv: (kv[0] is not None, kv[0] or "")
        ):
            if not series.points:
                continue
            if raw_portfolio is None or not raw_portfolio.strip():
                # A legacy snapshot with no usable portfolio key (None, empty
                # or whitespace-only). There is no safe portfolio identity to
                # derive a token from, so this series uses
                # the shared portfolio-less valuation account — the same
                # degradation as a missing installation secret. Its value is
                # still projected and still counted as represented.
                normalized = None
                token = None
            else:
            ...
            if normalized is not None:
                ...
                snapshot_backed_identities.add(normalized)
            else:
                null_source_projected = True
                snapshot_backed_identities.add(NULL_PORTFOLIO_LABEL)
            if not portfolio_entries:
                continue
            valuation_entries.extend(portfolio_entries)
            valuation_total += series.points[-1][1]
        if null_source_projected:
            represented_normalized.add(NULL_PORTFOLIO_LABEL)
        valuation_portfolios.extend(sorted(represented_normalized))
        # Coverage by UPLOAD ID, not by portfolio text. Every qualifying
        # snapshot yields a series and is represented by construction, so the
        # only thing left to report is an upload that no qualifying snapshot
        # backs. This removes the expected-vs-represented set comparison whose
        # mismatched identity types caused the previous scope defects; portfolio
        # text is now used for display only.
        # Kept by upload ID until serialization so two uncovered uploads sharing
        # a portfolio key produce TWO diagnostics, not one. A stable non-sensitive
        # ``#<n>`` suffix disambiguates duplicate display text; the upload id
        # itself is not exposed.
        uncovered = sorted(
            (display_key, upload_id)
            for upload_id, display_key in (
                scope_sources.cas_upload_display_keys.items()
            )
            if upload_id not in uploads_with_snapshots
        )
        seen_display: dict[str, int] = {}
        for display_key, _upload_id in uncovered:
            seen_display[display_key] = seen_display.get(display_key, 0) + 1
            occurrence = seen_display[display_key]
            total = sum(1 for key, _ in uncovered if key == display_key)
            valuation_unrepresented.append(
                display_key if total == 1 else f"{display_key} #{occurrence}"
            )
    ...
    if not cas_sources_present:
        ...
        # Compared against the SAME identity model the represented set uses:
        # snapshot-derived identities plus any upload no snapshot covers. This
        # is exactly ``represented | unrepresented``, so scope is "included"
        # precisely when nothing was left unrepresented.
        cas_investment_scope = "included" if not valuation_unrepresented else "partial"
