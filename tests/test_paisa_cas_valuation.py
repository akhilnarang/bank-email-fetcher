...
# ---------------------------------------------------------------------------
# Null portfolio keys: one native source, not one per upload
# ---------------------------------------------------------------------------


async def _null_key_cas(session, *, statement_date, grand_total, upload_key):
    """A CAS upload whose BalanceSnapshot carries no portfolio key.

    ``BalanceSnapshot.portfolio_key`` is nullable and its investment uniqueness
    index is partial (non-null only), so legacy/imported rows really can be
    null. ``networth._source_key`` reads the snapshot column only, so every such
    row shares ONE native source and native keeps just the latest.
    """
    upload = CasUpload(
        portfolio_key=upload_key,
        depository_source="cdsl",
        investor_name="Synthetic Portfolio",
        statement_date=statement_date,
        grand_total=Decimal(grand_total),
...
async def test_null_key_snapshots_follow_native_latest_source(session):
    """Different dates: native keeps the latest, so the ledger must too.

    Substituting ``CasUpload.portfolio_key`` split these into separate series
    and summed their histories, over-reporting against native.
    """
    from financial_dashboard.services.networth import current_networth
    ...
    for as_of in (datetime.date(2026, 4, 15), datetime.date(2026, 6, 30)):
        summary = await current_networth(session, today=as_of)
        native = sum(
            (
                row.value
    ...
    # No safe portfolio identity exists, so the shared account is used and no
    # upload key — relational metadata only — reaches the journal.
    assert INVESTMENT_VALUATION_ROOT in report.journal
    assert "PAN-A" not in report.journal
    assert "PAN-B" not in report.journal
    # The value IS represented, so it must not be reported as missing.
    assert report.investment_valuation_unrepresented == ()
    assert report.investment_valuation_portfolios == ("(no portfolio key)",)


async def test_null_key_same_date_latest_row_id_wins(session):
    """Same date: last row id wins, matching ``_latest_per_source``."""
    from financial_dashboard.services.networth import current_networth
    ...
    # Not 13000: one source, latest row id wins — exactly as native resolves it.
    assert native == Decimal("9000.00")
    assert _balance_at(report, INVESTMENT_VALUATION_ROOT, as_of) == native
...
async def test_valuation_kind_is_inside_the_closed_taxonomy(session):
    """Every emitted entry's kind must be in the exported closed set.

    ``DASHBOARD_KINDS`` is consumed by downstream validation, so omitting
    ``KIND_VALUATION`` would let it reject a valid generated valuation entry.
    """
    from financial_dashboard.services.paisa.projection import DASHBOARD_KINDS

    assert KIND_VALUATION in DASHBOARD_KINDS
    ...
    assert any(e.kind == KIND_VALUATION for e in report.entries)
    for entry in report.entries:
        assert entry.kind in DASHBOARD_KINDS, entry.kind
    for kind in report.kind_counts:
        assert kind in DASHBOARD_KINDS, kind


# ---------------------------------------------------------------------------
# One identity model for expected vs represented scope
# ---------------------------------------------------------------------------
#
# Expected and represented must be built by the SAME rule. Mixing upload-derived
# keys with snapshot-derived identities made a projected null-key source report
# "partial" while listing nothing as unrepresented — the value was in the ledger
# but the scope said otherwise.


async def test_null_key_source_reports_fully_included(session):
    await _bank_and_snapshot(session)
    await _null_key_cas(
        session,
        statement_date=datetime.date(2026, 3, 31),
...
async def test_null_key_source_does_not_mask_an_uncovered_upload(session):
    """A null source must not make an unrelated upload look represented."""
    await _bank_and_snapshot(session)
    await _null_key_cas(
        session,
        statement_date=datetime.date(2026, 3, 31),
    ...
    # A different upload with no BalanceSnapshot at all.
    session.add(
        CasUpload(
            portfolio_key="PAN-Z",
            depository_source="cdsl",
    ...
    assert report.cas_investment_scope == "partial"
    assert report.net_worth_sources_complete is False
    assert "PAN-Z" in report.investment_valuation_unrepresented
    # The null source itself is still represented.
    assert "(no portfolio key)" in report.investment_valuation_portfolios


async def test_null_key_source_with_all_zero_history_is_represented(session):
    await _bank_and_snapshot(session)
    await _null_key_cas(
        session,
        statement_date=datetime.date(2026, 3, 31),
...
# ---------------------------------------------------------------------------
# Restored from the preserved branch: live aggregate-only behaviour
# ---------------------------------------------------------------------------


async def test_no_unresolved_legs_yields_complete_scope(session):
    """The exactness gate must remain reachable, not permanently false."""
    await _bank_and_snapshot(session)
    await _persist_cas(
        session,
        payload=_demat_payload(holdings=[_holding()]),
...
async def test_all_zero_portfolio_is_represented_without_an_entry(session):
    await _bank_and_snapshot(session)
    await _persist_cas(
        session,
        payload=_demat_payload(holdings=[]),
...
def test_portfolio_token_varies_by_secret_and_normalizes_input():
    assert portfolio_token(PORTFOLIO_A, SECRET) != portfolio_token(
        PORTFOLIO_A, "b" * 64
    )
    assert portfolio_token("  synthetic-a  ", SECRET) == portfolio_token(
        PORTFOLIO_A, SECRET
    )
    assert portfolio_token(PORTFOLIO_A, "   ") is None
    assert normalize_portfolio_key("  synthetic-a  ") == PORTFOLIO_A


async def test_same_isin_in_two_accounts_keeps_two_diagnostic_identities(session):
    """Identity-preserving holding facts: the same ISIN in two demat accounts."""
    await _bank_and_snapshot(session)
    await _persist_cas(
        session,
        payload={
            "accounts": [
                {
                    ...
                    "client_id": "CL-1",
                    "depository": "cdsl",
                    "holdings": [_holding(value="25000")],
                },
                {
                    "dp_id": "DP-2",
                    "client_id": "CL-2",
                    "depository": "nsdl",
                    "holdings": [_holding(value="10000")],
                },
            ]
        },
        statement_date=datetime.date(2026, 3, 31),
    ...
    # Two source identities preserved, not collapsed by ISIN...
    assert report.investment_current_valuation_count == 2
    assert len(set(report.investment_valuation_sources)) == 2
    # ...while the aggregate carries the full stated value exactly once.
    assert report.investment_valuation_total == Decimal("35000.00")


# ---------------------------------------------------------------------------
# Coverage is per upload ID; every qualifying snapshot is represented
# ---------------------------------------------------------------------------
#
# Portfolio text is display only. Two uploads can share a key while only one has
# a snapshot, so keying coverage on that text marked both covered and hid the
# uncovered one behind an older value. Tracking cas_upload_id also removes the
# expected-vs-represented set comparison whose mismatched identity types caused
# the earlier scope defects.


async def _bare_upload(session, *, key, date, total):
    upload = CasUpload(
        portfolio_key=key,
        depository_source="cdsl",
        investor_name="Synthetic Portfolio",
...
async def _snapshot_for(session, *, upload, key, date, total):
    session.add(
        BalanceSnapshot(
            cas_upload_id=upload.id,
            portfolio_key=key,
...
async def _assert_matches_native(session, report, as_of):
    from financial_dashboard.services.networth import current_networth
...
async def test_blank_snapshot_key_is_projected_not_dropped(session, blank):
    """An empty/whitespace key is a real native source whose value counts.

    Dropping it removed the value from the journal while its upload was still
    marked covered — the report claimed complete scope over a missing balance.
    """
    await _bank_and_snapshot(session)
    upload = await _bare_upload(
        session, key="PAN-A", date=datetime.date(2026, 3, 31), total="10000.00"
    )
    await _snapshot_for(
        session,
        upload=upload,
    ...
    native = await _assert_matches_native(session, report, datetime.date(2026, 6, 30))
    assert native == Decimal("10000.00")
    assert report.cas_investment_scope == "included"
    assert report.investment_valuation_unrepresented == ()
    assert report.investment_valuation_portfolios == ("(no portfolio key)",)
    # No safe token, so the shared account is used and no raw key is emitted.
    assert INVESTMENT_VALUATION_ROOT in report.journal
    assert "PAN-A" not in report.journal


async def test_blank_key_plus_uncovered_upload_reports_the_uncovered_one(session):
    await _bank_and_snapshot(session)
    upload = await _bare_upload(
        session, key="PAN-A", date=datetime.date(2026, 3, 31), total="10000.00"
    )
    ...
    await _bare_upload(
        session, key="PAN-Z", date=datetime.date(2026, 4, 30), total="5000.00"
    )
...
async def test_same_key_only_older_upload_has_a_snapshot(session):
    await _bank_and_snapshot(session)
    older = await _bare_upload(
        session, key="PAN-A", date=datetime.date(2026, 3, 31), total="10000.00"
    )
    await _snapshot_for(
        session,
    ...
    # Newer upload, same key, no snapshot — must NOT be hidden by the older one.
    await _bare_upload(
        session, key="PAN-A", date=datetime.date(2026, 6, 30), total="99000.00"
    )

    report = await project(session, _config())

    await _assert_matches_native(session, report, datetime.date(2026, 9, 30))
    assert report.cas_investment_scope == "partial"
    assert report.investment_valuation_unrepresented == ("PAN-A",)
    assert "PAN-A" not in report.journal


async def test_same_key_only_newer_upload_has_a_snapshot(session):
    await _bank_and_snapshot(session)
    await _bare_upload(
        session, key="PAN-A", date=datetime.date(2026, 3, 31), total="10000.00"
    )
    newer = await _bare_upload(
        session, key="PAN-A", date=datetime.date(2026, 6, 30), total="12000.00"
    )
    await _snapshot_for(
        session,
        upload=newer,
...
async def test_same_key_both_uploads_have_snapshots(session):
    await _bank_and_snapshot(session)
    for date, total in (
        (datetime.date(2026, 3, 31), "10000.00"),
        (datetime.date(2026, 6, 30), "12000.00"),
    ):
        upload = await _bare_upload(session, key="PAN-A", date=date, total=total)
        await _snapshot_for(session, upload=upload, key="PAN-A", date=date, total=total)
    ...
    assert report.cas_investment_scope == "included"
    assert report.investment_valuation_unrepresented == ()
    assert report.investment_valuation_portfolios == ("PAN-A",)
    assert "PAN-A" not in report.journal


# ---------------------------------------------------------------------------
# Native source-selection parity: orphans, currency ordering, winner coverage
# ---------------------------------------------------------------------------


async def _snapshot_row(session, *, upload_id, key, date, total, currency="INR"):
    session.add(
        BalanceSnapshot(
            cas_upload_id=upload_id,
            portfolio_key=key,
            kind=SnapshotKind.asset.value,
            category=SnapshotCategory.investment.value,
            as_of_date=date,
...
async def test_orphan_snapshot_activates_projection(session):
    """A snapshot whose CasUpload row is missing still carries native value.

    SQLite does not enforce the foreign key, so an orphan row can exist. Gating
    projection on loaded uploads alone reported CAS scope "none" and complete
    while omitting that value entirely.
    """
    await _bank_and_snapshot(session)
    await _snapshot_row(
        session,
        upload_id=9999,  # no such CasUpload
        key="PAN-ORPHAN",
        date=datetime.date(2026, 3, 31),
        total="8000.00",
    )
...
async def test_newer_non_inr_row_clears_the_older_inr_value(session):
    """Native filters currency AFTER selecting the latest row per source.

    Filtering to INR in SQL left an older INR value standing that native had
    already dropped, over-reporting the ledger.
    """
    await _bank_and_snapshot(session)
    upload = await _bare_upload(
        session, key="PAN-A", date=datetime.date(2026, 3, 31), total="10000.00"
    ...
    native = await _assert_matches_native(session, report, datetime.date(2026, 8, 31))
    assert native == Decimal("0.00")


async def test_same_date_higher_id_non_inr_row_wins_and_clears(session):
    """Same date: the highest row id is authoritative even when non-INR."""
    await _bank_and_snapshot(session)
    upload = await _bare_upload(
        session, key="PAN-A", date=datetime.date(2026, 3, 31), total="10000.00"
    )
    # Null keys avoid the partial unique index while sharing one native source.
    await _snapshot_row(
        session,
        upload_id=upload.id,
        key=None,
...
async def test_same_date_loser_upload_is_not_marked_covered(session):
    """Coverage comes from the winner only, never a superseded row."""
    await _bank_and_snapshot(session)
    loser = await _bare_upload(
        session, key="PAN-A", date=datetime.date(2026, 3, 31), total="4000.00"
    )
    winner = await _bare_upload(
        session, key="PAN-B", date=datetime.date(2026, 3, 31), total="9000.00"
    )
    await _snapshot_row(
        session,
        upload_id=loser.id,
    ...
    await _assert_matches_native(session, report, datetime.date(2026, 6, 30))
    # The losing row is not authoritative, so its upload is still uncovered.
    assert report.cas_investment_scope == "partial"
    assert "PAN-A" in report.investment_valuation_unrepresented


async def test_duplicate_uncovered_uploads_each_get_a_diagnostic(session):
    """Two uncovered uploads sharing a key must not collapse into one item."""
    await _bank_and_snapshot(session)
    await _bare_upload(
        session, key="PAN-A", date=datetime.date(2026, 3, 31), total="1.00"
    )
    await _bare_upload(
        session, key="PAN-A", date=datetime.date(2026, 6, 30), total="2.00"
    )

    report = await project(session, _config())

    assert len(report.investment_valuation_unrepresented) == 2
    assert report.investment_valuation_unrepresented == ("PAN-A #1", "PAN-A #2")
    assert report.cas_investment_scope == "partial"


async def test_all_uploads_missing_snapshots_still_surface_in_the_summary(session):
    """The main preview must render the missing-value diagnostic on its own.

    It used to be nested inside the projected-portfolios branch, so an
    all-missing projection showed only a generic partial status.
    """
    from financial_dashboard.services.paisa.surface import _projection_summary
    ...
    assert summary is not None
    assert summary.investment_valuation_portfolios == []
    assert summary.investment_valuation_unrepresented == ["PAN-A"]
    assert summary.cas_investment_scope == "partial"
